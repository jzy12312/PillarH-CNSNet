...
class PillarFeatureNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 with_height: Optional[bool] = True,):
        ...
        if with_distance:
            in_channels += 1
        if with_height:  # 如果启用高度编码，增加一个通道
            in_channels += 1
        self._with_distance = with_distance
        self._with_cluster_center = with_cluster_center
        self._with_voxel_center = with_voxel_center
        self._with_height = with_height  # 添加高度编码标志
    ...
    def forward(self, features: Tensor, num_points: Tensor, coors: Tensor,
        ...
        if self._with_height:
            # 计算每个点的归一化高度
            height_min = self.point_cloud_range[2]
            height_max = self.point_cloud_range[5]
            height_range = height_max - height_min
            normalized_height = (features[:, :, 2] - height_min) / height_range
            normalized_height = normalized_height.unsqueeze(-1)  # 增加通道维度
            features_ls.append(normalized_height)
