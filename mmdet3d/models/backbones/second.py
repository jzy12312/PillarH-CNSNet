...
from mokuai.ConvNeX1 import ConvNeXt
...
class SECOND(BaseModule):
    ...
    def __init__(self,
        ...
        blocks = []
        convnext_blocks = []  # 用于存储ConvNeXt模块
        for i, layer_num in enumerate(layer_nums):
            # 添加ConvNeXt模 ConvNeXtV2块
            convnext_block =  ConvNeXt(dim=in_filters[i])
            convnext_blocks.append(convnext_block)
            block = [build_conv_layer(conv_cfg,
                                      in_filters[i],
                                      out_channels[i],
                                      3,
        ...
        self.convnext_blocks = nn.ModuleList(convnext_blocks)  # 注册ConvNeXt模块
