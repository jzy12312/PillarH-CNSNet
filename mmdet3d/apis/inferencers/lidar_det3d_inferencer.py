...
class LidarDet3DInferencer(Base3DInferencer):
    ...
    def visualize(self,
        ...
        if getattr(self, 'visualizer') is None:
            ...
            # 1) 类别 → 颜色映射表（Open3D 用 0~1 浮点 RGB）
        CLASS_COLOR = {
            'car': [0.0, 1.0, 0.0],  # 绿色
            'cyclist': [1.0, 0.0, 0.0],  # 红色
            'pedestrian': [0.0, 0.0, 1.0],  # 蓝色（兜底）
        }
        ...
        for single_input, pred in zip(inputs, preds):
            ...
            if img_out_dir != '' and show:
                ...
                o3d_save_path = None
                # 2) 为每个实例生成颜色（按类别名）
                # 注意：pred.labels_3d 可能是 int 或 str
                labels = pred.labels_3d
                if isinstance(labels, torch.Tensor):
                    labels = labels.cpu().numpy()
                class_names = self.model.dataset_meta['classes']
                pred_colors = []
                for lb in labels:
                    name = class_names[lb] if isinstance(lb, (int, np.integer)) else str(lb)
                    pred_colors.append(CLASS_COLOR.get(name, [1.0, 1.0, 1.0]))

                # 将颜色列表挂到 pred 里，可视化器会自动使用
                pred.bbox_color = pred_colors  # 关键一行
